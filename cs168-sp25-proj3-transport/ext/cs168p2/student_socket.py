...
class StudentUSocket(StudentUSocketBase):
  ...
  def close(self):
    ...
    if self.state is CLOSED:
      ...
      ## Start of Stage 6.2 ##
      self.fin_ctrl.set_pending(next_state=LAST_ACK) # we want to send a FIN later
      ## End of Stage 6.2 ##
      pass
  ...
  def handle_accepted_payload(self, payload):
    ...
    ## Start of Stage 2.3 ##
    rcv.nxt = rcv.nxt |PLUS| len(payload)   # this is accepted, in-order payload, so update recv space
    rcv.wnd = rcv.wnd |MINUS| len(payload)  # decrease the recv window because the received payload 
  ...
  def handle_accepted_fin(self, seg):
    ...
    if self.state == ESTABLISHED:
      # passive close
      self.rcv.nxt = self.rcv.nxt |PLUS| 1  # update the next seq num expect to receive
